...
def run_simulation(gradient, density, p, p0, pchange, pchange_slow, psurr, psurr_slow):
    ...
    if not exists('simulation_'+confname):
        print('Recompiling')
        comp = subprocess.run(['g++','-O2','simulation_profile.cpp','-o','simulation_'+confname])

    # we run the command
    seed = 0
    print(['./simulation_'+confname,str(seed), "%d"%density, "%.2f"%p, "%.2f"%(p0), "%.2f"%(pchange), "%.2f"%(pchange_slow), "%.2f"%(psurr), "%.2f"%(psurr_slow),"1"])
    subprocess.Popen(['./simulation_'+confname,str(seed),"%d"%density, "%.2f"%p, "%.2f"%(p0), "%.2f"%(pchange), "%.2f"%(pchange_slow), "%.2f"%(psurr), "%.2f"%(psurr_slow),"1"])
...
def run_simulation_parallel(Nsim, gradient, density, p, p0, pchange, pchange_slow, psurr, psurr_slow, cardata):
    ...
    for i in range(Nsim):
        ...
        if ((i == 0) and cardata):
            procs.append(subprocess.Popen(['./simulation_'+confname, str(i), "%d"%density, "%.2f"%p, "%.2f"%(p0), "%.2f"%(pchange), "%.2f"%(pchange_slow), "%.2f"%(psurr), "%.2f"%(psurr_slow),"1"]))
        else:
            procs.append(subprocess.Popen(['./simulation_'+confname, str(i), "%d"%density, "%.2f"%p, "%.2f"%(p0), "%.2f"%(pchange), "%.2f"%(pchange_slow), "%.2f"%(psurr), "%.2f"%(psurr_slow),"0"]))
